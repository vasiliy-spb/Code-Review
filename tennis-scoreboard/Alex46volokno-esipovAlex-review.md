...
- в таблице итоговых матчей, если у одного из игроков слишком длинное им, то текст выходит за пределы таблицы ![](images/Alex46volokno-esipovAlex/Screenshot_02.png)
- при попытке поиска игрока по части имени появляется страница с ошибкой и возможностью вернуться только на главную страницу. ![](images/Alex46volokno-esipovAlex/Screenshot_03.png) Вместо этого лучше показывать пустой список
- нет кнопки сброса фильтра по имени на странице завершённых матчей, если оставить поле ввода пустым и нажать `найти` (попытаться сбросить фильтр), то возникнет ошибка ![](images/Alex46volokno-esipovAlex/Screenshot_04.png) Вместо этого лучше показывать полный (не фильтрованный) список матчей 
...
- при переходе с домашней страницы на страницу с итогами матчей выскакивает окно "Подтвердите действие на странице" ![](images/Alex46volokno-esipovAlex/Screenshot_05.png)
- при вводе имени игрока длиннее 255 символов (столько по умолчанию помещается в строковое поле в БД H2) происходит ошибка и показывается соответствующая страница ![](images/Alex46volokno-esipovAlex/Screenshot_06.png) Вместо этого лучше просто сказать пользователю (показать текст над полем имени), что имя слишком длинное
...
- если два игрока сыграют два матча и поочереди в них победят, то в списке матчей счёт будет показываться как `2 - 0` и `2 - 0`, вместо `2 - 0` и `0 - 2` ![](images/Alex46volokno-esipovAlex/Screenshot_07.png)
- если id матча помещается в одну строку в колонке `№ Матча`, то разметка таблицы матчей тоже ломается ![](images/Alex46volokno-esipovAlex/Screenshot_10.png)
